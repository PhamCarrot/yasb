...
class TransportControls(QFrame):
    def __init__(self, scale, parent=None, *, popup=False, compact=False):
        super().__init__(parent)
        ...
        self.previous = TransportButton("previous", s(side), s(21 if popup else 16), self)
        self.play = TransportButton("play", s(play), s(32 if popup else 18), self)
        self.next = TransportButton("next", s(side), s(21 if popup else 16), self)
        for button in (self.previous, self.play, self.next):
            layout.addWidget(button, 0, Qt.AlignmentFlag.AlignVCenter)
            button.setEnabled(False)
        self.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)

    def set_metrics(self, *, scale=None, compact=False, maximum_size=None):
        ...
        sizes = (round(side * scale), round(play * scale), round(side * scale))
        if maximum_size is not None:
            sizes = tuple(min(int(maximum_size), value) for value in sizes)
        icons = (
            round((21 if self._popup else 16) * scale),
            round((32 if self._popup else 18) * scale),
            round((21 if self._popup else 16) * scale),
        )
        for button, size, icon in zip((self.previous, self.play, self.next), sizes, icons):
            button.setFixedSize(size, size)
            button.icon_size = min(icon, max(8, size - 4))
            button.update()
